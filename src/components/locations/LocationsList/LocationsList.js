...
function LocationsList({ items }) {
  return (
    <table className="table table-bordered table-hover table-striped table-responsive">
      ...
      <tbody>
      {items.map(({ objectId, name, address, phone, website, category, neighborhood, metro_city, reservations, rating, groups, outdoor, location_type, createdAt, tags }) => (
        <tr key={objectId}>
          <td>{name}</td>
          <td>{address}</td>
          <td>{phone}</td>
          ...
          <td>
			...
			{location_type ? location_type.name: null }
          </td>
