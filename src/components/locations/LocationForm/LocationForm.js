...
class LocationForm extends Component {
  ...
  handleInitialize() {
    const {
      item,
      item: {
        objectId, yelp_id, name, address, phone, website, category, neighborhood, opentable_id, metro_city, metro_city2, hours, estimate_cost, reservations,
        latitude, longitude, rating, notes, additional_notes, groups, outdoor, location_type, verified, 
        tags, ticket_url, yelp_reservation, skip_automation
      },
      initialize
    } = this.props;
    ...
    if (!isEmpty(item)) {
      initialize({
        objectId, yelp_id, name, address, phone, website, category, neighborhood, opentable_id, metro_city,
        metro_city2,
        hours, estimate_cost, reservations, latitude, longitude, rating, notes, additional_notes, groups, outdoor, location_type, verified,
        tags, ticket_url, yelp_reservation, skip_automation
      });
    } else {
      initialize({
        ...
        <div className="row">
          <div className="col-md-6">
            <Field
              ...
              label="Add from Yelp"
              onSelect={({ id, alias, name, categories, location, display_phone, rating, coordinates, hours, neighborhoods }) => initialize({
                yelp_id: alias,
                name: name,
			...
			<Field name="website" component={renderField} label="Website" />
            <Field name="category" component={renderField} label="Category" />
            <Field name="tags" component={renderMultiselect} data={tags.map(({ tag }) => tag)} label="Tags" />
            ...
            <Field name="opentable_id" component={renderField} label="Opentable ID" />
			      <Field name="notes" component={renderTextareaField} label="Notes" min="1" max="100" />
          </div>
          <div className="col-md-6">
            ...
            <Field name="additional_notes" component={renderTextareaField} label="Additional Notes" min="1" />
            <Field name="verified" component={renderCheckboxField} label="Verified?"/>
            <Field name="ticket_url" component={renderField} label="Ticket URL"/>
            <Field name="yelp_reservation" component={renderCheckboxField} label="Yelp Reservation"/>
            <Field name="skip_automation" component={renderCheckboxField} label="Skip Automation"/>
          </div>
        </div>
        {errorMessage ? (
...
function validate({ yelp_id, location_type, notes, additional_notes, ...rest }) {
  const errors = {};
  ...
  if (!isEmpty(rest)) {
    ...
    if (!location_type) {
      ...
      errors.notes = 'Note is required';
  		...
  		errors.notes = 'Maximum 100 characters are allowed';
  	}
  }
...
export default connect(({
  ...
  tags: { items: tags }
}) => ({ locationTypes, tags }), ({ fetchLocationTypes, fetchTags }))(reduxForm({ form: 'location', validate, asyncValidate })(LocationForm));