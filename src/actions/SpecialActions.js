  ...
  return dispatch => apiRequest.post('Special', {
    ...
    end_date: special.end_date ? {
      ...
      iso: special.end_date
		...
		location_pointer: special.location_pointer ? {
      __type: 'Pointer',
			className: 'Location',
      objectId: special.location_pointer.objectId
    } : null,
  })
    .then((response) => { 
		//{special_pointer: {__type: 'Pointer', className: 'Special',objectId: response.data.objectId }};
		apiRequest.put('Location', special.location_pointer.objectId, {special: true}).then(() =>{ 
		browserHistory.push('/specials');
		});
	})
...
export function updateSpecial(itemID, special) {
  return dispatch => apiRequest.put('Special', itemID, {
    ...
    .then((response) => {
		//{special_pointer: {__type: 'Pointer', className: 'Special',objectId: itemID }}
		apiRequest.put('Location', special.location_pointer.objectId, {special: true}).then(() =>{ browserHistory.push('/specials');
		});
	})
    .catch(({ response: { data: { error } } }) => dispatch(specialError(error)));
}
