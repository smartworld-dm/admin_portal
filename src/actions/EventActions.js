...
export function createEvent(event, { objectId }) {
  return dispatch => apiRequest.post('Event', {
    ...
    .then(() => {
		if(event.location){
			apiRequest.put('Location', event.location.objectId, {event: true}).then(() =>{ 
...
export function batchCreateEvents(events, { objectId, location }){
        ...
        let singleRequest = {
            ...
            body: {
                ...
                location: events[i].location ? {
                    __type: 'Pointer',
                    className: 'Location',
                    objectId: events[i].location.objectId
                } : null,
                special: event.special ? {
                    __type: 'Pointer',
                    className: 'Special',
                ...
                event_type: events[i].event_type ? {
                    __type: 'Pointer',
                    className: 'EventType',
                    objectId: events[i].event_type.objectId
                } : null
            }
