...
import { ADD_LOCATIONS, ADD_VLOCATIONS, ADD_LOCATION, LOCATION_ERROR, SHOW_LOCATION, REMOVE_LOCATION } from '../constants/Location';
...
export function addLocationsVerified(itemsVerified = [], countVerified = 0) {
  return {
    type: ADD_VLOCATIONS,
    itemsVerified,
    countVerified
...
export function fetchLocations({ search, include, order, limit }) {
  const url = [
    'Location?count=1',
...
export function fetchLocationsVerified({ search, include, order, limit }) {
  const url = [
    'Location?count=1',
    order ? `&order=${order}` : null,
    include ? `&include=${include}` : null,
    `&where=${JSON.stringify({
        $and: [
		  ...
		  { location_type: null }
		  //{ 
		    //$or : [ {_p_location_type: { $exists : false} }, {location_type: null } ]
		  //},
        ]
      })}`,
			limit ? `&limit=${limit}` : `&limit=count`
  ...
  return dispatch => apiRequest.get(url)
    .then(({ data: { results, count } }) => dispatch(addLocationsVerified(results, count)));
}
...
export function updateLocation(itemID, location, onlyOne) {
  if (!onlyOne)
